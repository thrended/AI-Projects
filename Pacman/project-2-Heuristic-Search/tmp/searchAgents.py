...
=======
        ...
        # self.cornerList = {(1,1): False, (1,top): False, (right, 1): False, (right, top): False}
        self.costFn = lambda x,y: 1
        self.found = [False] * 4
        self.startingState = (self.startingPosition, self.found)
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
            ...
            if not self.walls[nextx][nexty]:
                point = (nextx, nexty)
                successive_corner = list(state[1]) 
                if point in self.corners:
                    if point not in successive_corner:
                        successive_corner.append(point)
                successor = ((point, successive_corner), action, 1)
...
=======
            ...
            cornerList = state[1]
            cornerListSucc = cornerList[:]
            if not hitsWall:
                for corner in self.corners:
                    if (nextx, nexty) == corner:
                        cornerListSucc[self.corners.index(corner)] = True
                costFn = self.costFn(nextx, nexty)
                nextState = ( (nextx, nexty), cornerList[:] )
                successors.append( ( nextState, action, costFn ) )
>>>>>>> Stashed changes
...
def cornersHeuristic(state, problem):
    ...
    heuristic = [0]
    for point in unvisited_corners:
...
def foodHeuristic(state, problem):
    ...
    food_coordicates = food_grid.asList()
